...
- modulation via `nb:modulate(val)` -> pairs well with sidvagn. I surfaced the `mod depth` parameter, so it's midi mappable, which means modulation can be used with scripts that don't use `nb:modulate`.
- fx via fx mod
- 6 voice limit
